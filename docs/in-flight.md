...
| C18 | **The round-up chips are said to spill out of their cards, and I cannot reproduce it.** Jorge's screenshot shows the `+10` and `-10` chips crossing the card border. Injecting the chip into the real cards and sweeping the viewport puts it **70 to 86px INSIDE** the card at every width from 780 to 1440. A `flex-wrap` fix was written and then taken back out rather than shipped as a proven fix for an unproven cause. Needs from Jorge: which screen and how wide, and whether it is the chips or the cards that are spilling. | Claude | - | `src/Manager.jsx` | waiting on Jorge | - |
