...
| C10 | **New and used are not in the report at all.** The `.3` and `.7` are real and deliberate. DriveCentric's summary grid carries `deliveries: 74` and units by CHANNEL (phone 13, internet 30, showroom 26, campaign 5) and **no stock type at all**. The only new/used figures are per-person credited ones, which double count split deals: 21 people at Holler Ford credit 107.5 new+used between them against 74 delivered. `vehicleSplit` scales the mix onto the real total, `Math.round(x*10)/10`, and that is where the tenths come from. So the bar is an estimated mix drawn as if it were a count. | Claude | - | `src/Manager.jsx`, the report | needs approval | - |
