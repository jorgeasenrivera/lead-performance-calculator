...
| C10 | **New and used are in the report, and thrown away three lines before they are used.** I got this wrong first time: I read the parsed blob in the database, saw units by channel only, and said the report had no stock type. Jorge's screenshot of the summary grid shows New and Used rows on the store's own block. Reading the parser: `DS_VEHICLE` is `["New","Used","Other","Total"]`, those rows take the same branch as the channel rows, and on the store's block (`!curName`) they are kept as `storeSources.new` and `storeSources.used`. Then `dcUnits` reads only internet, phone, showroom and campaign, so `stated` never carries them and they die in that function. With no stated split, `vehicleSplit` falls back to scaling per-person credited figures, which double count split deals, and the scaling to one decimal is what makes 36.3 and 37.7. The fix is to carry the store's new/used/other into `stated` and prefer them. | Claude | `claude/mobile-site-optimization-qvei7u` | `api/_report-parsers.mjs`, `src/Manager.jsx` | in progress | - |
