...
| C3 | **Drop `public.queue_identity`.** Six rows, last touched 22 August, and nothing reads or writes it since the PIN came out in #326. Jorge said go ahead on 15 September. A migration under `supabase/migrations/`, nothing else. | Claude | `claude/mobile-site-optimization-qvei7u` | `supabase/migrations/` | in review | #339 |
