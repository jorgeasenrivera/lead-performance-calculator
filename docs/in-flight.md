...
| C18 | **The round-up chips are said to spill out of their cards, and it is still not reproducible.** Second pass after Jorge said 100% zoom: the cards are a **fixed 201px at every viewport from 1280 to 2560**, so window width and zoom cannot be the variable. Measured on **all four sides** with the chip injected: everything sits 10 to 12px INSIDE the card. Below 620px the round-up does not render at all. Tested in Chromium only, which is the one hypothesis left: if Jorge is on Safari the chip's metrics could differ. Needs: a wider screenshot showing the whole sheet, and which browser. | Claude | - | `src/Manager.jsx` | waiting on Jorge | - |
