...
| A5 | **The roster card's sizing, and the Live Activity card.** One CSS rule, `.s2g4 svg`, was written as a descendant selector and caught the 9px verdict glyphs as well as the dial, drawing them at 68x34. Labels spilled onto their neighbours and the row ran off the card. Fixed, with the wrap breakpoint, the dock clearance and the demo seed's units. The Live Activity is diagnosed but not changed: it asks for about 298pt of a lock screen that allows about 160. | Claude | `claude/mobile-site-optimization-qvei7u` | `src/Manager.jsx`, `scripts/demo-seed.mjs` | needs approval | - |
