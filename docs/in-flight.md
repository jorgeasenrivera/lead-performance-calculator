...
| C13 | **The sold line runs through its own caption.** `.mc-tl` is `position:absolute; bottom:-14px` with three parts on one row, so when it wraps the block grows UPWARD into the chart. Measured at 390px with a real caption: Normal 1 line and clear, **Large 2 lines and 11.9px into the chart, Largest 3 lines and 13.8px**. A 360px phone hits it at Normal. Needs Jorge's pick between stacking, shortening and shrinking. | Claude | - | `src/LeadPerformanceCalculator.jsx` | needs approval | - |
| C14 | **Fold the help icon into the initials, and put the initials on the spine's axis.** The sheet behind the initials already has two rows that open the same help panel, so the question mark is a second door to a room you can already enter. Reverses an earlier decision still recorded in a code comment (consistency pass, item 8), which is flagged on the proposal rather than quietly dropped. The spine's centre line is 18px from the edge; the initials and the question mark sit on two other axes. | Claude | - | `src/LeadPerformanceCalculator.jsx` | needs approval | - |
| C15 | **The ground does not travel between rooms.** The rooms already slide, dim and sheen; the background is swapped instantly underneath. Home `#15211B`, Live Floor `#070A08`, Phone Line `#06090F`. Proposal is to crossfade the ground over the same `--t-wipe` as the slide, with a louder option where the ground leads. | Claude | - | `src/LeadPerformanceCalculator.jsx` | needs approval | - |
| C16 | **Swipe to change rooms.** The switch is already directional (`--ar-dx`, `arPageIn`/`arPageOut`); only the gesture is missing, and nothing in the app listens for one. Two shapes offered: follows the thumb, or a flick reusing the existing switch. Three things to respect: sideways scrollers inside rooms must win, iOS edge-back must be left alone, and a gesture that fights a scroll is worse than none. Do last, on its own. | Claude | - | `src/LeadPerformanceCalculator.jsx` | needs approval | - |
