...
| J5 | **The Expo free-plan builds are spent for the month.** Nothing native can be built until the plan is upgraded on expo.dev (jorgeasenrivs-team → Billing) or the month resets. Only #340 is waiting on it; web changes need no build. | waiting on Jorge |
